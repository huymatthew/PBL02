...
Bill::Bill() : bill_id(0), contract_id(0), billing_month(""), room_rent(0.0),
    ...
    if (pk_manager.isKeyInUse(bill_id)) {
        bill_id = pk_manager.getNextKey();
    } else {
        pk_manager.addKey(bill_id);
    }
}
...
vector<Bill> Bill::loadFromDatabase() {
    vector<Bill> bills;
    // Code to load bills from the database
    return bills;
}

bool Bill::saveToDatabase(const Bill& bill) {
    // Code to save the bill to the database
